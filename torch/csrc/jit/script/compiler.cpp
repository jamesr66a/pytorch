...
void defineMethodsInModule(Module & m, const std::vector<DefAndTypes>& definitions, const std::vector<Resolver>& resolvers, SugaredValuePtr self) {
  FunctionTable table;
  JIT_ASSERT(definitions.size() == resolvers.size());
  auto resolver_it = resolvers.begin();
  std::vector<Method*> methods;
  for(DefAndTypes def : definitions) {
    const std::string& name = def.def.name().name();
...
std::shared_ptr<Module> compileFunction(DefAndTypes def, const Resolver& resolver) {
  std::shared_ptr<Module> m; //note: we don't use 'm' to execute so this setting is unused
